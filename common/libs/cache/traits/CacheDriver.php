...
trait CacheDriver {
    ...
    protected function init($config) {
        ...
        if (isset($config['fallback']) && $config['fallback'] === false) {
            ...
            $this->fallback = empty($config['fallback']) ? \libs\cache\Cache::getConfig('fallback') : $config['fallback'];
        }
        //自定义延迟过期时间
    ...
    public function backup() {
        ...
        return \libs\cache\Cache::getInstance($fallback);
    }
